...
export const updateProducer = async (req: Request, res: Response) => {
  try {
    ...
    if (parseInt(id) !== loggedProducer.id) {
      return res
        .status(403)
        .json({
          error: "Acesso proibido. Você só pode atualizar seu próprio perfil.",
        });
    }
  ...
  try {
    ...
    if (parseInt(id) !== loggedProducer.id) {
      return res
        .status(403)
        .json({
          error: "Acesso proibido. Você só pode alterar sua própria senha.",
        });
    }
  ...
  try {
    ...
    if (parseInt(id) !== loggedProducer.id) {
      return res
        .status(403)
        .json({
          error: "Acesso proibido. Você só pode deletar seu próprio perfil.",
        });
    }
